...
class AngsuranTable extends AbstractTableGateway {
    ...
    public function __construct(Adapter $adapter) {
        ...
        $this->resultSetPrototype->setArrayObjectPrototype(new AngsuranBase());
        $this->initialize();
    }

    public function getAngsuranId($t_idangsuran) {
        $rowset = $this->select(array('t_idangsuran' => $t_idangsuran));
        $row = $rowset->current();
        return $row;
    }
    ...
    public function simpanangsuran(AngsuranBase $base, $session, $post, $i) {
        $no = $this->maxNoAngsuran();

        $t_noangsuran = (int) $no['t_noangsuran'] + 1;
        ...
        $data = array(
            ...
            't_noangsuran' => $t_noangsuran,
            't_tglketetapanangsuran' => date('Y-m-d', strtotime($base->t_tglketetapanangsuran)),
            't_jumlahkaliangsuran' => $base->t_jumlahkaliangsuran,
            't_tgljadwalangsuran' => date('Y-m-d', strtotime($post['t_tgljadwalangsuran'][$i])),
            't_nominalangsuran' => intval(str_ireplace('.', '', $post['t_nominalangsuran'][$i])),
            't_angsuranke' => $post['t_angsuranke'][$i],
            't_kodebayarangsuran' => $kdProvkabkota."" . str_pad($base->t_jenispajak, 2, "0", STR_PAD_LEFT) . "11" . substr(date('Y', strtotime($base->t_tglketetapanangsuran)), 2, 2)."".str_pad($t_noangsuran, 6, "0", STR_PAD_LEFT),
//            't_kodebayarangsuran' =>  . "11" . str_pad(, 6, "0", STR_PAD_LEFT),
            't_inputangsuran' => $session['s_iduser']
        );
       // var_dump($data);exit();
        $t_angsuran = new \Zend\Db\TableGateway\TableGateway('t_angsuran', $this->adapter);
        $t_angsuran->insert($data);
        return $data;
    }

    public function getGridCountKetetapanAngsuranSkpd(\Pajak\Model\Pendataan\PendataanBase $base) {
        $sql = new Sql($this->adapter);
        $select = $sql->select();
        $select->from(array(
            "a" => "view_wp"
        ...
        $where = new Where();
        $where->literal("d.t_tglketetapanangsuran is not null and d.t_jenisketetapan=2");
    ...
    public function getGridDataKetetapanAngsuranSkpd(\Pajak\Model\Pendataan\PendataanBase $base, $offset) {
        $sql = new Sql($this->adapter);
        $select = $sql->select();
        $select->from(array(
            "a" => "view_wp"
    ...
    public function getGridCountKetetapanAngsuranSkpdkb(\Pajak\Model\Pendataan\PendataanBase $base) {
        $sql = new Sql($this->adapter);
        $select = $sql->select();
        $select->from(array(
            "a" => "view_wp"
        ...
        $where = new Where();
        $where->literal("e.t_tglketetapanangsuran is not null and e.t_jenisketetapan=5");
        if ($base->kolomcari != 'undefined') {
            if ($base->combocari != "undefined") {
                if ($base->combooperator == "carilike" || $base->combooperator == 'undefined') {
                    $where->literal("$base->combocari ILIKE '%$base->kolomcari%'");
    ...
    public function getGridDataKetetapanAngsuranSkpdkb(\Pajak\Model\Pendataan\PendataanBase $base, $offset) {
        $sql = new Sql($this->adapter);
        $select = $sql->select();
        $select->from(array(
            "a" => "view_wp"
    ...
    public function getGridCountKetetapanAngsuranSkpdkbt(\Pajak\Model\Pendataan\PendataanBase $base) {
        $sql = new Sql($this->adapter);
        $select = $sql->select();
        $select->from(array(
            "a" => "view_wp"
        ...
        $where = new Where();
        $where->literal("e.t_tglketetapanangsuran is not null ");
        if ($base->kolomcari != 'undefined') {
            if ($base->combocari != "undefined") {
                if ($base->combooperator == "carilike" || $base->combooperator == 'undefined') {
                    $where->literal("$base->combocari ILIKE '%$base->kolomcari%'");
    ...
    public function getGridDataKetetapanAngsuranSkpdkbt(\Pajak\Model\Pendataan\PendataanBase $base, $offset) {
        $sql = new Sql($this->adapter);
        $select = $sql->select();
        $select->from(array(
            "a" => "view_wp"
    ...
    public function getGridCountKetetapanAngsuranSkpdt(\Pajak\Model\Pendataan\PendataanBase $base) {
        $sql = new Sql($this->adapter);
        $select = $sql->select();
        $select->from(array(
            "a" => "view_wp"
    ...
    public function getGridDataKetetapanAngsuranSkpdt(\Pajak\Model\Pendataan\PendataanBase $base, $offset) {
        $sql = new Sql($this->adapter);
        $select = $sql->select();
        $select->from(array(
            "a" => "view_wp"
...
//        $where->literal("a.t_tglpembayaran is null and a.t_jenispajak in (4,8) and c.t_idangsuran is null");
        $where->literal("a.t_tglpembayaran is null and a.t_jenispajak not in (1,2,3,5,6,7,9) and c.t_idangsuran is null");
...
//        $where->literal("a.t_tglpembayaran is null and a.t_jenispajak in (4,8) and c.t_idangsuran is null");
    ...
    public function getDataKetetapanForAngsuran($t_idketetapan, $t_jenisketetapan) {
        ...
        if ($t_jenisketetapan == 2) {
            ...
            $select->columns(array(
                "t_idwpobjek", "t_idtransaksi", "t_nopenetapan", "t_jmlhpajak", 't_tglpenetapan', "t_tgljatuhtempo", "t_jenispajak"
            ));
            $select->join(array(
                "b" => "view_wpobjek"
                    ), "b.t_idobjek = a.t_idwpobjek", array(
    ...
    public function getDatabyIDAngsuranSkpd($t_idangsuran, $t_jenisketetapan) {
        ...
        $select->columns(array(
            "t_noangsuran", "t_nominalangsuran", "t_kodebayarangsuran", "t_jumlahkaliangsuran", "t_tglketetapanangsuran", "t_idketetapan", "t_angsuranke", "t_tgljadwalangsuran"
        ));
        $select->join(array(
            "b" => "t_transaksi"
        ...
        $select->join(array(
            ...
            "korek", "s_namakorek", "s_persentarifkorek"
                ), $select::JOIN_LEFT);
        $select->join(array(
            "e" => "s_jenissurat"
    ...
    public function getDatabyIDAngsuranSkpdkb($t_idangsuran, $t_jenisketetapan) {
        ...
        $select->columns(array(
            "t_noangsuran", "t_nominalangsuran", "t_kodebayarangsuran", "t_jumlahkaliangsuran", "t_tglketetapanangsuran", "t_idketetapan", "t_angsuranke"
        ));
        $select->join(array(
            "b" => "t_skpdkb"
