...
class DinasTable extends AbstractTableGateway {

    protected $table = 's_dinas';

    public function __construct(Adapter $adapter) {
        $this->adapter = $adapter;
        $this->resultSetPrototype = new ResultSet();
        $this->resultSetPrototype->setArrayObjectPrototype(new DinasBase());
        $this->initialize();
    }
    ...
    public function savedata(DinasBase $kb, $session) {
        $data = array(
            's_kodedinas' => $kb->s_kodedinas,
            's_namadinas' => $kb->s_namadinas,
            'jalan_dinas' => $kb->jalan_dinas,
            's_idkec' => $kb->s_idkec,
            's_idkel' => $kb->s_idkel
        );
        $id = (int) $kb->s_iddinas;
        if ($id == 0) {
            ...
            $this->update($data, array('s_iddinas' => $kb->s_iddinas));
        }
    }

    

    public function getDinasId($s_iddinas) {
        /*$rowset = $this->select(array('s_iddinas' => $s_iddinas));
        $row = $rowset->current();
        return $row;*/
        ...
        $where->literal('s_iddinas = '.$s_iddinas.'');
        $select->where($where);
        $state = $sql->prepareStatementForSqlObject($select);
    ...
    public function hapusData($id) {
        $this->delete(array('s_iddinas' => $id));
    }
    ...
    public function semuadata_dinas($input, $aColumns, $session, $cekurl, $allParams) {
        ...
        foreach ($rResult as $aRow) {
            ...
            $btn = '<a href="setting_dinas/edit?s_iddinas='.$aRow['s_iddinas'].'" class="btn btn-warning btn-xs btn-flat"><i class="fa fa-pencil"></i> Edit</a> <a href="#" onclick="hapus('.$aRow['s_iddinas'].');return false;" class="btn btn-danger btn-xs btn-flat"><i class="fa fa-trash"></i> Hapus</a>';
