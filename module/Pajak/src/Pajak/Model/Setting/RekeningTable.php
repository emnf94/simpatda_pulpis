...
class RekeningTable extends AbstractTableGateway
{
    ...
    public function getGridDataRekening(RekeningBase $base, $start, $parametercari)
    {
        ...
        if ($parametercari->t_jenisobjek == 5) {
            $where->isNotNull('s_persentarifkorek');
        } else {
            $where->notEqualTo('s_sub1korek', '000');
        }
        $select->limit($base->rows = (int) $base->rows);
    ...
    public function getGridDataRekeningTarget(RekeningBase $base, $start, $parametercari)
    {
        ...
        $select = $sql->select();
        $select->from("s_rekening");
        $select->join("s_targetdetail", "s_targetdetail.s_targetrekening = s_rekening.s_idkorek", [
            "s_idtargetdetail",
    ...
    public function getdataRekeningMineral()
    {
        ...
        $where->equalTo('s_jeniskorek', '01');
        $where->equalTo('s_persentarifkorek', '20');
        $where->notEqualTo('s_rinciankorek', '00');
        // $where->literal("s_sub1korek::text !='' ");
        $where->literal("s_sub2korek::text ='00' ");
        $select->where($where);
        $select->order(['s_rinciankorek' => 'asc']);
        $state = $sql->prepareStatementForSqlObject($select);
        $res = $state->execute();
    ...
    public function getdataRekeningsubMineral($t_subjeniskorek)
    {
        $sql = new Sql($this->adapter);
        $select = $sql->select();
        $select->from('view_rekening');
        ...
        // $where->literal("s_sub1korek::text !='' ");
        $where->literal("s_sub2korek::text !='00' ");
        // $where->literal("s_rinciankorek::text ='$t_subjeniskorek' ");
        $select->where($where);
        $select->order(['s_rinciankorek' => 'asc']);
        $state = $sql->prepareStatementForSqlObject($select);
        $res = $state->execute();
    ...
    public function comboBoxRekening()
    {
        $sql = "select * from s_rekening "
            ...
            . "and s_persentarifkorek notnull "
            . "and s_jeniskorek = '01' "
            . "and s_sub1korek = '001' "
    ...
    public function getDataIdMinerbabyrincian($s_rinciankorek)
    {
        $sql = "select * from view_rekeningminerba where s_rinciankorek = '" . $s_rinciankorek . "' and s_sub2korek='00' and s_persentarifkorek=20 order by s_rinciankorek asc";

        $statement = $this->adapter->query($sql);
        return $statement->execute()->current();
    }

    public function getdataJenisMinerba($idjenisminerba)
    {
        $sql = "select * from view_rekeningminerba where s_rinciankorek = '" . $idjenisminerba . "' and s_sub2korek!='00' order by s_rinciankorek asc";

        $statement = $this->adapter->query($sql);
        return $statement->execute();
    }

    public function getDataRekeningMblb()
    {
        $sql = "select * from view_rekeningminerba where s_objekkorek = '14' and s_sub1korek='001' and s_sub2korek='00' and s_persentarifkorek=20 order by s_rinciankorek asc";
    ...
    public function getdataJenisObjekOPD()
    {
        $sql = new Sql($this->adapter);
        $select = $sql->select();
        $select->from('view_rekening');
        $where = new Where();
        // $where->literal("s_idjenis is not NULL");
        $where->notEqualTo('s_sub1korek', '000');
        $select->where($where);
        $select->order("korek asc");
        $state = $sql->prepareStatementForSqlObject($select);
    ...
    public function getRekeningIdkorek($s_idkorek)
    {
        $sql = "select * from view_rekening where s_idkorek = '$s_idkorek'";
        $statement = $this->adapter->query($sql);
        return $statement->execute()->current();
    }


    public function getRekeningpokok($s_tipekorek, $s_jeniskorek, $s_objekkorek, $s_rinciankorek)
    {
        $sql = "select * from view_rekening where s_tipekorek = '$s_tipekorek' AND s_jeniskorek='$s_jeniskorek' AND s_objekkorek='$s_objekkorek' AND s_rinciankorek='$s_rinciankorek' AND s_sub1korek='000'";
        $statement = $this->adapter->query($sql);
