...
class KeberatanTable extends AbstractTableGateway {

    protected $table = 't_keberatan';

    public function __construct(Adapter $adapter) {
        $this->adapter = $adapter;
        $this->resultSetPrototype = new ResultSet();
        $this->resultSetPrototype->setArrayObjectPrototype(new KeberatanBase());
        $this->initialize();
    }

    public function getKeberatanId($t_idkeberatan) {
        $rowset = $this->select(array('t_idkeberatan' => $t_idkeberatan));
        $row = $rowset->current();
        return $row;
    }
    ...
    public function maxNoKeberatan() {
        $sql = "select max(t_nokeberatan) as t_nokeberatan from t_keberatan ";
        $statement = $this->adapter->query($sql);
        $res = $statement->execute();
        return $res->current();
    }

    public function simpankeberatan(KeberatanBase $base, $session, $post) {

        $no = $this->maxNoKeberatan();
        $t_nokeberatan = (int) $no['t_nokeberatan'] + 1;
        $data = array(
            ...
            't_tglketetapankeberatan' => date('Y-m-d', strtotime($base->t_tglketetapankeberatan)),
            't_alasankeberatan' => $base->t_alasankeberatan,
            't_jmlhketetapanseharusnya' => (int)(str_ireplace('.', '', $base->t_jmlhketetapanseharusnya)),
            't_inputkeberatan' => $session['s_iduser'],
            't_jmlhpengurangan' => 0,
            't_jmlhditetapkan' => 0,
            
        );
    //reklame
        if ((int)$base->t_jenispajak==4) {
            $data['t_jangkawaktu']=(int)$base->t_jangkawaktu;
            $data['t_ukuranmedia']=(int)$base->t_ukuranmedia;
            $data['t_parameter']=$base->t_parameter;
            $data['t_jumlah']=(int)$base->t_jumlah;
            $data['t_jenisreklameusaha']=(int)$base->t_jenisreklameusaha;
           
        }
        //air
        if ((int)$base->t_jenispajak == 8) {
            $data['t_debitair']=(int)(str_ireplace('.', '',$base->t_debitair));
            $data['t_lamawaktu']=(int)$base->t_lamawaktu;
            $data['t_totalbiaya']=(int)(str_ireplace('.', '', $base->t_totalbiaya));
            $data['t_kualitasair']=(int)$base->t_kualitasair;
            $data['t_peruntukan']=(int)$base->t_peruntukan;
            $data['t_volume']=(int)(str_ireplace('.', '', $base->t_volume));
        }

        $t_keberatan = new \Zend\Db\TableGateway\TableGateway('t_keberatan', $this->adapter);
        if(empty($base->t_idkeberatan)){
            $data['t_nokeberatan'] = $t_nokeberatan;
            $t_keberatan->insert($data);
        }else{
            $t_keberatan->update($data, array('t_idkeberatan' => $base->t_idkeberatan));
        }
        
    }

    public function simpanPersetujuan(PersetujuanKeberatanBase $base, $session, $post) {
        // var_dump($post);exit();
        $data = array(
            't_tglverifikasi' => date('Y-m-d', strtotime($base->t_tglverifikasi)),
            't_nilaipengurangan' => (int) $base->t_nilaipengurangan,
            't_jmlhketetapanseharusnya' => (int) str_ireplace('.', '', $base->t_jmlhpajak),
            't_jmlhpengurangan' => (int) (str_ireplace('.', '', $base->t_jmlhpengurangan)),
            't_jmlhditetapkan' => str_ireplace('.', '', $base->t_jmlhditetapkan),
            't_pejabatverifikasi' => $session['s_iduser'],
            't_statusverifikasi' => $base->t_persetujuan,
			't_nomorsk' => $base->t_nomorsk,
            't_restitusi'=> (int) (str_ireplace('.', '', $post['t_restitusi'])),
        );

        // var_dump($data);exit();

        $t_keberatanpsj = new \Zend\Db\TableGateway\TableGateway('t_keberatan', $this->adapter);
        $t_keberatanpsj->update($data, array('t_idkeberatan' => $base->t_idkeberatan));
        // var_dump($data);exit();
        // if((int)$base->t_jenisketetapan == 2){
        //     $datatransaksi = array(
        //         't_jmlhpajak' => str_ireplace('.', '', $base->t_jmlhditetapkan)
        //     );
        //     $t_keberatantransaksi = new \Zend\Db\TableGateway\TableGateway('t_transaksi', $this->adapter);
        //     $t_keberatantransaksi->update($datatransaksi, array('t_idtransaksi' => $base->t_idketetapan));
        if((int)$base->t_jenisketetapan == 5){
            $datatransaksi = array(
                't_jmlhpajakskpdkb' => str_ireplace('.', '', $base->t_jmlhditetapkan)
            );
            $t_keberatantransaksi = new \Zend\Db\TableGateway\TableGateway('t_skpdkb', $this->adapter);
            $t_keberatantransaksi->update($datatransaksi, array('t_idskpdkb' => $base->t_idketetapan));
        }elseif((int)$base->t_jenisketetapan == 6){
            $datatransaksi = array(
                't_jmlhpajakskpdkbt' => str_ireplace('.', '', $base->t_jmlhditetapkan)
            );
            $t_keberatantransaksi = new \Zend\Db\TableGateway\TableGateway('t_skpdkbt', $this->adapter);
            $t_keberatantransaksi->update($datatransaksi, array('t_idskpdkbt' => $base->t_idketetapan));
        }
...
//    }
    ...
    public function getGridCount(KeberatanBase $base, $post) {
        $sql = new Sql($this->adapter);
        $select = $sql->select();
        $select->from(array(
            "a" => "t_keberatan"
        ...
        if ($post->t_tgldaftar != '') {
            ...
            $where->literal("a.t_tglketetapankeberatan between '" . date('Y-m-d', strtotime($t_tgl[0])) . "' and '" . date('Y-m-d', strtotime($t_tgl[1])) . "'");
        }
        if ($post->t_npwpd != '')
    ...
    public function getGridData(KeberatanBase $base, $offset, $post) {
        $sql = new Sql($this->adapter);
        $select = $sql->select();
        $select->from(array(
            "a" => "t_keberatan"
        ...
        $select->where($where);
        $select->order("a.t_idkeberatan desc");
    ...
    public function getGridCountsudah(KeberatanBase $base, $post) {
        $sql = new Sql($this->adapter);
        $select = $sql->select();
        $select->from(array(
            "a" => "t_keberatan"
    ...
    public function getGridDatasudah(KeberatanBase $base, $offset, $post) {
        $sql = new Sql($this->adapter);
        $select = $sql->select();
        $select->from(array(
            "a" => "t_keberatan"
    ...
    public function getGridCountSkpd(KeberatanBase $base, $parametercari) {
        $sql = new Sql($this->adapter);
        $select = $sql->select();
        $select->from(array(
            "a" => "t_transaksi"
...
//      $where->literal("a.t_tglpembayaran is null and a.t_jenispajak in (4,8) and c.t_idkeberatan is null");
        $where->literal("a.t_tglpembayaran is not null and a.t_jenispajak not in (1,2,3,5,6,7,9) and c.t_idkeberatan is null");
    ...
    public function getGridDataSkpd(KeberatanBase $base, $offset, $parametercari) {
        $sql = new Sql($this->adapter);
        $select = $sql->select();
        $select->from(array(
            "a" => "t_transaksi"
...
//        $where->literal("a.t_tglpembayaran is null and a.t_jenispajak in (4,8) and c.t_idkeberatan is null");
    ...
    public function getDataKetetapanForKeberatan($t_idketetapan, $t_jenisketetapan) {
        $sql = new Sql($this->adapter);
        $select = $sql->select();
        $where = new Where();
        if ($t_jenisketetapan == 2) {
            ...
            $select->columns(array(
                "t_idwpobjek", "t_idtransaksi", "t_nopenetapan", "t_jmlhpajak", 't_tglpenetapan', "t_tgljatuhtempo", "t_jenispajak","t_tarifpajak","t_dasarpengenaan"
            ));
            $select->join(array(
                "b" => "view_wpobjek"
                    ), "b.t_idobjek = a.t_idwpobjek", array(
    ...
    public function getDataKeberatanID($t_idkeberatan) {
        $sql = new Sql($this->adapter);
        $select = $sql->select();
        $select->from(array(
            "a" => "t_keberatan"
        ...
        $where = new Where();
        $where->equalTo('a.t_idkeberatan', $t_idkeberatan);
        $select->where($where);
        $state = $sql->prepareStatementForSqlObject($select);
        $res = $state->execute()->current();
        return $res;
    ...
    public function getDatabyIDKeberatanSkpd($t_idkeberatan, $t_jenisketetapan) {
        $sql = new Sql($this->adapter);
        $select = $sql->select();
        $select->from(array(
            "a" => "t_keberatan"
        ...
        $select->join(array(
            ...
            "t_idtransaksi", "t_jmlhpajak", "t_nopenetapan", "t_tglpenetapan", "t_tgljatuhtempo", "t_periodepajak"
                ), $select::JOIN_LEFT);
        $select->join(array(
            "c" => "view_wpobjek"
                ), "b.t_idwpobjek = c.t_idobjek", array(
    ...
    public function getDatabyIDKeberatanSkpdkb($t_idkeberatan, $t_jenisketetapan) {
        $sql = new Sql($this->adapter);
        $select = $sql->select();
    ...
    public function getDataIdKeberatan($t_idkeberatan, $t_jenisketetapan) {
        $sql = new Sql($this->adapter);
        $select = $sql->select();
        $select->from(array(
            "a" => "t_keberatan"
        ...
        if($t_jenisketetapan == 2){
            ...
            "t_nopenetapan", "t_tglpenetapan", "t_tgljatuhtempo", "t_jmlhpajak","t_tarifpajak","t_idtransaksi"
                ), $select::JOIN_LEFT);
        }elseif($t_jenisketetapan == 5){
            $select->join(array(
            "c" => "t_skpdkb"
        ...
        $where = new Where();
        $where->equalTo('a.t_idkeberatan', (int) $t_idkeberatan);
        $select->where($where);
        $state = $sql->prepareStatementForSqlObject($select);
        $res = $state->execute()->current();
        return $res;
    ...
    public function getDataReklame($t_idtransaksi){
        $sql = new Sql($this->adapter);
        $select = $sql->select();
        $select->from(array(
            "a" => "t_detailreklame"
        ...
        $where = new Where();
        $where->equalTo('a.t_idtransaksi', $t_idtransaksi);
        $select->where($where);
        $state = $sql->prepareStatementForSqlObject($select);
        $res = $state->execute()->current();
        return $res;
    ...
    public function getDataAir($t_idtransaksi){
        $sql = new Sql($this->adapter);
        $select = $sql->select();
        $select->from(array(
            "a" => "t_detailair"
