...
left join view_wp_v3 t3 ON t3.t_idwp=t2.t_idwp
...
order by t0.t_kodebayar, t1.s_tipekorek, t1.s_kelompokkorek, t1.s_jeniskorek, t1.s_objekkorek, t1.s_rinciankorek";
    ...
    public function getdataBpps($tgl1, $tgl2, $viabayar)
    {
        ...
        $sql =
            ...
            // LEFT JOIN view_wp_v3 ON view_wp.t_idwp=view_wpobjek.t_idwp
...
where extract(month from t0.t_tglbayardenda)::int = " . $bulan . " AND extract(year from t0.t_tglbayardenda)::int = " . $tahun . "
    ...
    public function getdataRealisasi($tglawal, $tglakhir, $idrealisasi)
    {
        ...
        $cekurl = "$_SERVER[HTTP_HOST]";
        if ($cekurl == "localhost:86") {
            $connect_db_bphtb = "host=localhost user=postgres password=postgres dbname=bphtb_pulpis"; //KONEKSI DB BPHTB
        } else {
            $connect_db_bphtb = "host=localhost user=postgres password=bphtb12345 dbname=bphtb_pulpis"; //KONEKSI DB BPHTB
        }
        ...
        $select->columns(array(
            ...
            "s_sub2korek",
            "s_sub3korek",
            "korek" => new \Zend\Db\Sql\Expression(
                "(
                CASE WHEN " . "s_kelompokkorek = '0' THEN s_tipekorek " .
            ...
            'jmltarget' => new \Zend\Db\Sql\Expression("CASE
         		...
         		WHEN 
         		s_rekening.s_sub1korek = '000' or s_rekening.s_sub1korek = '0'  THEN
         		(
         		SELECT SUM
         		( s_targetjumlah ) 
         		FROM
            ...
            "transaksi_blnlalu" => new \Zend\Db\Sql\Expression("(case when s_rekening.s_kelompokkorek != '0' THEN " .
                ...
                "case when s_rekening.s_sub1korek != '000' THEN " .
            ...
            "setoranlain_sdblnini" => new \Zend\Db\Sql\Expression("(case when s_rekening.s_kelompokkorek != '0' THEN " .
                ...
                ") " .
                "END )"),
            // "bphtb_blnlalu" => new \Zend\Db\Sql\Expression("0"),
            // "bphtb_blnini" => new \Zend\Db\Sql\Expression("0"),
            // "bphtb_sdblnini" => new \Zend\Db\Sql\Expression("0"),
            ...
            "bphtb_blnlalu" => new \Zend\Db\Sql\Expression("(case when s_rekening.s_kelompokkorek != '0' THEN " .
                ...
                "case when s_rekening.s_sub2korek != '0' THEN " .
                "case when s_rekening.s_sub3korek != '0' THEN " .
                "(SELECT COALESCE(SUM(bphtb.t_jmlhbayar_total),0) 
	FROM dblink('" . $connect_db_bphtb . "','SELECT t_tglpembayaran_pokok,t_jmlhbayar_total FROM t_pembayaran_bphtb ') AS bphtb(t_tglpembayaran_pokok date, t_jmlhbayar_total bigint) " .
                "LEFT JOIN s_rekening za on za.s_idkorek = 252 " .
                "WHERE to_char(bphtb.t_tglpembayaran_pokok, 'YYYYMM') < '" . date('Ym', strtotime($tglakhir)) . "' " .
                "AND EXTRACT ( YEAR FROM bphtb.t_tglpembayaran_pokok ) = '" . $tahun . "' " .
                "AND za.s_tipekorek = s_rekening.s_tipekorek " .
                "AND za.s_kelompokkorek = s_rekening.s_kelompokkorek " .
                "AND za.s_jeniskorek = s_rekening.s_jeniskorek " .
                "AND za.s_objekkorek = s_rekening.s_objekkorek " .
	...
	FROM dblink('" . $connect_db_bphtb . "','SELECT t_tglpembayaran_pokok,t_jmlhbayar_total FROM t_pembayaran_bphtb ') AS bphtb(t_tglpembayaran_pokok date, t_jmlhbayar_total bigint) " .
                ...
                "WHERE to_char(bphtb.t_tglpembayaran_pokok, 'YYYYMM') < '" . date('Ym', strtotime($tglakhir)) . "' " .
                "AND EXTRACT ( YEAR FROM bphtb.t_tglpembayaran_pokok ) = '" . $tahun . "' " .
                // "WHERE bphtb.t_tglpembayaran_pokok >= '" . date('Y-m-d', strtotime($tglawal)) . "' and bphtb.t_tglpembayaran_pokok <= '" . $tgl_akhir . "' " .
                "AND za.s_tipekorek = s_rekening.s_tipekorek " .
                "AND za.s_kelompokkorek = s_rekening.s_kelompokkorek " .
                "AND za.s_jeniskorek = s_rekening.s_jeniskorek " .
                "AND za.s_objekkorek = s_rekening.s_objekkorek " .
	...
	FROM dblink('" . $connect_db_bphtb . "','SELECT t_tglpembayaran_pokok,t_jmlhbayar_total FROM t_pembayaran_bphtb ') AS bphtb(t_tglpembayaran_pokok date, t_jmlhbayar_total bigint) " .
                "LEFT JOIN s_rekening za on za.s_idkorek = 252 " .
                "WHERE bphtb.t_tglpembayaran_pokok >= '" . date('Y-m-01', strtotime($tglakhir)) . "' and bphtb.t_tglpembayaran_pokok <= '" . date('Y-m-d', strtotime($tglakhir)) . "' " .
                "AND za.s_tipekorek = s_rekening.s_tipekorek " .
                "AND za.s_kelompokkorek = s_rekening.s_kelompokkorek " .
                "AND za.s_jeniskorek = s_rekening.s_jeniskorek " .
                "AND za.s_objekkorek = s_rekening.s_objekkorek " .
	...
	FROM dblink('" . $connect_db_bphtb . "','SELECT t_tglpembayaran_pokok,t_jmlhbayar_total FROM t_pembayaran_bphtb ') AS bphtb(t_tglpembayaran_pokok date, t_jmlhbayar_total bigint) " .
                "LEFT JOIN s_rekening za on za.s_idkorek = 252 " .
                "WHERE bphtb.t_tglpembayaran_pokok <= '" . date('Y-m-d', strtotime($tglakhir)) . "' " .
                "AND EXTRACT ( YEAR FROM bphtb.t_tglpembayaran_pokok ) = '" . $tahun . "' " .
                "AND za.s_tipekorek = s_rekening.s_tipekorek " .
                "AND za.s_kelompokkorek = s_rekening.s_kelompokkorek " .
                "AND za.s_jeniskorek = s_rekening.s_jeniskorek " .
