...
class Setoranlain extends AbstractActionController
{
    ...
    public function dataGridAction()
    {
        ...
        $page = $base->page;
        $limit = $base->rows;
        $count = $this->Tools()->getService('SetoranlainTable')->getGridCount($base, $parametercari);
        if ($count > 0 && $limit > 0) {
        ...
        if ($start < 0)
            $start = 0;
        $data = $this->Tools()->getService('SetoranlainTable')->getGridData($base, $start, $parametercari);
        $s = "";
        ...
        foreach ($data as $row) {
            ...
            $s .= "<td data-title='Jumlah Setor' style='text-align: right'>" . number_format($row['t_jumlahsetor'], 2, ',', '.') . "</td>";
        ...
        if ($session['s_akses'] != 2 && $session['s_akses'] != 6) {
            ...
            if ($this->getRequest()->isGet()) :
                ...
                if ($data_setor['t_idsetoranlain'] != NULL) :
                    ...
                    $form->get('t_jumlahsetor')->setValue(number_format($data_setor['t_jumlahsetor'], 2, ',', '.'));
