...
namespace Pajak\Controller\Pratama;

use Zend\Mvc\Controller\AbstractActionController;
use Zend\View\Model\ViewModel;

class Pratama extends AbstractActionController {
    ...
    public function dataGridAction() {
        ...
        $page = $base->page;
        $limit = $base->rows;
        $count = $this->Tools()->getService('PendataanTable')->getGridCountAllPajak($base);
        if ($count > 0 && $limit > 0) {
        ...
        if ($start < 0)
            $start = 0;
        $data = $this->Tools()->getService('PendataanTable')->getGridDataAllPajak($base, $start);
        $s = "";
        ...
        foreach ($data as $row) {
            ...
            $s .= "<td data-title='No. SPTPD'><center><a href='javascript:void(0)' onclick='bukaDetailPendataan(" . $row['t_idtransaksi'] . ")' class='btn btn-warning btn-xs'>" . $row['t_nourut'] . "</a></center></td>";
            ...
            if (empty($row['t_tglpembayaran'])) {
                ...
                $s .= "<td data-title='Status Bayar'><center><button class='btn btn-xs btn-round btn-primary' title='Sudah Bayar'><i class='glyph-icon icon-check'></i></button></center></td>";
            }
            $s .= "</tr>";
            $counter++;
        }
    ...
    public function dataPendataanAction() {
        ...
        $data = $this->Tools()->getService('PendataanTable')->getDataPendataanIDTransAll($data_get['idtransaksi']);
        ...
        $html .= "  <div class='content-box border-top border-green' style='font-size:11px'>
                    ...
                    <div class='content-box border-top border-green' style='font-size:11px'>
                        ...
                        <div class='content-box-wrapper'>
                            <div class='row'>
                                <div class='col-sm-12'>
                                    <div class='col-sm-2'>Tgl. Pendataan</div>
                                    <div class='col-sm-4'>
                                        : " . date('d-m-Y', strtotime($data['t_tglpendataan'])) . "
                                    </div>
                                </div>
                                <div class='col-sm-12'>
    ...
    public function cetakdataAction() {
        ...
        $data = $this->Tools()->getService('PendataanTable')->getDataAllPajak($data_get->t_kecamatan, $data_get->jenispajak);
