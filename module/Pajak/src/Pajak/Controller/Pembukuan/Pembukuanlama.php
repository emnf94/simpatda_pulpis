...
//        return $pdf;
    ...
    public function cetakpiutangAction() {
        ...
        $data = $this->Tools()->getService('PenagihanTable')->getDataPiutangSemuaperiode($data_get->periodepiutang, $data_get->t_kecamatanpiutang, $kodekelurahan, $data_get->jenisobjpiutang);
        // foreach ($data as $key) {
        //             # code...
        //             var_dump($key);
        //         }        
...
//                ->getActiveSheet()->getStyle('I6')->getBorders()->getRight()->setBorderStyle(\PHPExcel_Style_Border::BORDER_THIN);
        ...
        $object->setActiveSheetIndex(0)
                ...
                ->setCellValue('C5', 'Kode Bayar')
                ->setCellValue('D5', 'Diterima Dari')
                ->setCellValue('E5', 'NPWPD')
        ...
        foreach ($parent_korek['namakorek'] as $col_parent => $row_parent):
            ...
            foreach ($data as $row):
                if ($row['s_jenisobjek'] == $parent_korek['jeniskorek'][$col_parent]):
                    ...
                    $ex->setCellValue("C" . $counter, $row['t_kodebayar']);
                    $ex->setCellValue("D" . $counter, 'DITERIMA DARI ' . $row['t_namaobjek']);
                    $ex->setCellValue("E" . $counter, $row['t_npwpd']);
...
//        var_dump($data_pbb['total_tahunanpbb']); exit();
      ...
      public function cetaksetoranlainAction() {
       ...
       $tglbayar0=date('Y-m-d', strtotime($data_get->tahunsetoranlain.'-'.$data_get->bulansetoranlain.'-'.'01'));
        ...
        $dataopd = $this->Tools()->getService('PembukuanTable')->dataopd();
        foreach ($dataopd as $key ) {

        $bulanini = $this->Tools()->getService('PembukuanTable')->databulanini($tglbayar0, $tglbayar1,$key['s_idsatker']);
        $bulanlalu =$this->Tools()->getService('PembukuanTable')->databulanlalu($tglbayar0,$key['s_idsatker']);

        $sampaidenganbulanini = $bulanlalu['bulanlalu']+$bulanini['bulanini'];
        ...
        $ar_mengetahui = $this->Tools()->getService('PejabatTable')->getPejabatId($data_get->pejabatlaplain);
           ...
           $datatransaksi[]=array(
            'dataopd'=>$key,
            'sampaidenganbulanini'=>$sampaidenganbulanini,
            'bulanini'=>$bulanini['bulanini'],
            'bulanlalu'=>$bulanlalu['bulanlalu'],
            );
        }
