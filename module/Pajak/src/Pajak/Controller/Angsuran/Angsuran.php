...
namespace Pajak\Controller\Angsuran;

use Zend\Mvc\Controller\AbstractActionController;
use Zend\View\Model\ViewModel;
use Pajak\Form\Angsuran\AngsuranFrm;
use Pajak\Model\Angsuran\AngsuranBase;

class Angsuran extends AbstractActionController {
    ...
    public function dataGridKetetapanAngsuranSkpdAction() {
        $session = $this->getServiceLocator()->get('PajakService')->getStorage()->read();
        ...
        $page = $base->page;
        $limit = $base->rows;
        $count = $this->Tools()->getService('AngsuranTable')->getGridCountKetetapanAngsuranSkpd($base);
        if ($count > 0 && $limit > 0) {
        ...
        if ($start < 0)
            $start = 0;
        $data = $this->Tools()->getService('AngsuranTable')->getGridDataKetetapanAngsuranSkpd($base, $start);
        $s = "";
        ...
        foreach ($data as $row) {
            ...
            if ($row['t_tglpembayaranangsuran'] == null) {
                $s .= "<td style='color:red; font-weight:bold'>Belum Terbayar</td>";
            } else {
                $s .= "<td style='color:green; font-weight:bold'>Sudah Dibayar</td>";
            }
            $s .= "<td>
    ...
    public function dataGridKetetapanAngsuranSkpdkbAction() {
        $session = $this->getServiceLocator()->get('PajakService')->getStorage()->read();
        $allParams = (array) $this->getEvent()->getRouteMatch()->getParams();
        $base = new \Pajak\Model\Pendataan\PendataanBase();
        $base->exchangeArray($allParams);
        ...
        $page = $base->page;
        $limit = $base->rows;
        $count = $this->Tools()->getService('AngsuranTable')->getGridCountKetetapanAngsuranSkpdkb($base);
        if ($count > 0 && $limit > 0) {
            $total_pages = ceil($count / $limit);
        } else {
            $total_pages = 0;
        ...
        if ($start < 0)
            $start = 0;
        $data = $this->Tools()->getService('AngsuranTable')->getGridDataKetetapanAngsuranSkpdkb($base, $start);
        $s = "";
        $counter = 1 + ($base->page * $base->rows) - $base->rows;
        if ($counter <= 1) {
            $counter = 1;
    ...
    public function dataGridKetetapanAngsuranSkpdkbtAction() {
        $session = $this->getServiceLocator()->get('PajakService')->getStorage()->read();
        $allParams = (array) $this->getEvent()->getRouteMatch()->getParams();
        $base = new \Pajak\Model\Pendataan\PendataanBase();
        $base->exchangeArray($allParams);
        ...
        $page = $base->page;
        $limit = $base->rows;
        $count = $this->Tools()->getService('AngsuranTable')->getGridCountKetetapanAngsuranSkpdkbt($base);
        if ($count > 0 && $limit > 0) {
            $total_pages = ceil($count / $limit);
        } else {
            $total_pages = 0;
        ...
        if ($start < 0)
            $start = 0;
        $data = $this->Tools()->getService('AngsuranTable')->getGridDataKetetapanAngsuranSkpdkbt($base, $start);
        $s = "";
        $counter = 1 + ($base->page * $base->rows) - $base->rows;
        if ($counter <= 1) {
            $counter = 1;
        ...
        foreach ($data as $row) {
            $s .= "<tr>";
            $s .= "<td><center>" . $counter . "</center></td>";
            $s .= "<td><center>" . $row['t_nourut'] . "/" . $row['t_nopembayaran'] . "</center></td>";
            $s .= "<td><center>" . $row['t_npwpd'] . "</center></td>";
            $s .= "<td>" . $row['t_nama'] . "</td>";
//            $s .= "<td data-title='ID OP'><center>" . $row['t_nop'] . "</center></td>";
            $s .= "<td>" . $row['t_namaobjek'] . "</td>";
            $s .= "<td><center>" . date('d-m-Y', strtotime($row['t_masaawal'])) . " / " . date('d-m-Y', strtotime($row['t_masaakhir'])) . "</center></td>";
            $s .= "<td><center>" . date('d-m-Y', strtotime($row['t_tglpembayaran'])) . "</center></td>";
            $s .= "<td style='text-align: right'>" . number_format($row['t_dasarpengenaan'], 0, ',', '.') . "</td>";
            $s .= "<td style='text-align: right'>" . number_format($row['t_jmlhpembayaran'], 0, ',', '.') . "</td>";
            $s .= "<td>" . $row['s_namajenis'] . "</td>";
            $s .= "<td></td>";
            $s .= "</tr>";
            $counter++;
    ...
    public function dataGridKetetapanAngsuranSkpdtAction() {
        $session = $this->getServiceLocator()->get('PajakService')->getStorage()->read();
        $allParams = (array) $this->getEvent()->getRouteMatch()->getParams();
        $base = new \Pajak\Model\Pendataan\PendataanBase();
        $base->exchangeArray($allParams);
        ...
        $page = $base->page;
        $limit = $base->rows;
        $count = $this->Tools()->getService('AngsuranTable')->getGridCountKetetapanAngsuranSkpdt($base);
        if ($count > 0 && $limit > 0) {
            $total_pages = ceil($count / $limit);
        } else {
            $total_pages = 0;
        ...
        if ($start < 0)
            $start = 0;
        $data = $this->Tools()->getService('AngsuranTable')->getGridDataKetetapanAngsuranSkpdt($base, $start);
        $s = "";
        $counter = 1 + ($base->page * $base->rows) - $base->rows;
        if ($counter <= 1) {
            $counter = 1;
...
//            $s .= "<td data-title='ID OP'><center>" . $row['t_nop'] . "</center></td>";
    ...
    public function FormAngsuranAction() {
        $session = $this->getServiceLocator()->get('PajakService')->getStorage()->read();
        $req = $this->getRequest();
        $form = new AngsuranFrm();
        if ($this->getRequest()->isPost()) {
            $base = new AngsuranBase();
            $form->setInputFilter($base->getInputFilter());
            $post = $req->getPost()->toArray();
            $form->setData($post);
            if ($form->isValid()) {
                $base->exchangeArray($form->getData());
                for ($i = 0; $i < $base->t_jumlahkaliangsuran; $i++) {
                    $this->Tools()->getService('AngsuranTable')->simpanangsuran($base, $session, $post, $i);
                }
                return $this->redirect()->toRoute('angsuran');
            }
        }
        $view = new ViewModel(array(
            'form' => $form
    ...
    public function CariJumlahKaliAngsuranAction() {
        ...
        $rincianangsuran .= "       <table class='table table-bordered table-striped table-condensed cf' style='font-size : 11px; color:black'>
                                        <thead class='cf' style='background-color:blue'>
                                            <tr>
                                                <th colspan='6' style='background-color: #00BCA4; color: red; text-align:center;font-size:10pt;'><b>Rincian Angsuran</b></th>
                                            </tr
                                            <tr>
