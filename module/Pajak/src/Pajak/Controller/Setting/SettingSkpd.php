...
class SettingSkpd extends AbstractActionController {
    ...
    public function editAction() {
        ...
        $req = $this->getRequest();
        if ($req->isGet()) {
            $id = (int) $req->getQuery()->get('s_idskpd');
            $data = $this->Tools()->getService('SkpdTable')->getSkpdId($id);
            $data2 = new \Zend\Stdlib\ArrayObject;  
            $data2['s_idskpd'] = $data['s_idskpd'];
            $data2['s_namaskpd'] = $data['s_namaskpd'];
            $data2['jalan_skpd'] = $data['jalan_skpd'];
            $data2['s_idkec'] = $data['s_idkec'];
            $data2['s_idkel'] = $data['s_idkel'];
            
            $datakecamatan = $this->Tools()->getService('SkpdTable')->getcomboIdKecamatan();
            $datakelurahan = $this->comboKelurahanCamat($data2['s_idkec']);
