...
class CekTunggakan extends AbstractActionController
{
        ...
        foreach ($dataobjekpajak as $row) {
            $dataobjek .= "                 
                    <div class='panel'>
                        <div class='panel-body'>
                            <h3>
                                <b class='title-hero' style='color:blue;'>Data Objek " . $row['s_namajenis'] . "</b>                  
                            </h3>
            ...
            if ($row['s_idjenis'] != 4) {
                $dataobjek .= "<input type='button' width='100'class='btn-dat-objek-pajak transaksi' value='Transaksi' onclick='bukatransaksibyobjek(" . $row['t_idobjek'] . "," . $iddiv . ")'>";
                $dataobjek .= "<div class='cari'>
                ...
                $dataobjek .= "<div class='cari'>
                                    <label for='periode'>Periode</label>
                                    <input type='text' name='periode' class='periode' id='periodereklame" . $iddiv . "' value=" . $periodepajak . ">
                                    <input type='button' value='Cari' class='btn-dat-objek-pajak' id='cariperiode'onclick='cariTunggakanReklamePerPeriode(" . $iddiv . ", " . $row['t_idobjek'] . ")'>
                                </div>    
                            </div>";
            }
            ...
            if ($row['s_idjenis'] != 4) {
                $dataobjek .= "<div id='datatransaksi" . $iddiv . "'>
                                <div class='remove-columns' style='float: left; margin:2px auto;'>
                                    <table class='table table-bordered table-striped table-condensed cf' style='font-size: 11px; color: black; width:470px'>
                                        <thead class='cf' style='background-color: blue;'>
                                            <tr>
                ...
                $dataobjek .= "  <div class='example-box-wrapper'>
                                <div class='remove-columns'>
                                    <table class='table table-bordered table-striped table-condensed cf' style='font-size : 11px; color:black'>
                                        <thead class='cf' style='background-color:blue'>
                                            <tr>
                ...
                $datatransaksipajak = $this->Tools()->getService('ObjekTable')->getTransaksibyIdObjek($row['t_idobjek'], $periodepajak);

                $i = 1;
                foreach ($datatransaksipajak as $aa) {


                    $t_tglpenetapan = (!empty($aa['t_tglpenetapan']) ? date('d-m-Y', strtotime($aa['t_tglpenetapan'])) : '-');
                    $t_tglpembayaran = (!empty($aa['t_tglpembayaran']) ? date('d-m-Y', strtotime($aa['t_tglpembayaran'])) : '-');
                    $t_tgldendapembayaran = (!empty($aa['t_tgldendapembayaran']) ? date('d-m-Y', strtotime($aa['t_tgldendapembayaran'])) : '-');
                    $t_tglbayardenda = (!empty($aa['t_tglbayardenda']) ? date('d-m-Y', strtotime($aa['t_tglbayardenda'])) : '-');
                    $dataobjek .= "         <tr data-toggle='collapse' data-target='#accordion" . $aa['t_idtransaksi'] . "' class='clickable'>
                                                <td data-title='No.' style='text-align:center'>" . $i++ . "</td>
                                                <td data-title='Kode Rekening'>" . $aa['korek'] . " - " . $aa['s_namakorek'] . "</td>
                                                <td data-title='Tgl.' style='text-align:center'>" . date('d-m-Y', strtotime($aa['t_tglpendataan'])) . "</td>
                                                <td data-title='Masa Pajak' style='text-align:center'>" . date('d-m-Y', strtotime($aa['t_masaawal'])) . " s/d " . date('d-m-Y', strtotime($aa['t_masaakhir'])) . "</td>
                                                <td data-title='Pajak' style='text-align:right'>" . number_format($aa['t_jmlhpajak'], 0, ',', '.') . "</td>
                                                ...
                                                <td data-title='Jumlah' style='text-align:right'>" . number_format($aa['t_jmlhpembayaran'], 0, ',', '.') . "</td>
                                                <td data-title='Detail' style='text-align:right'><button type='button' class='btn btn-xs btn-warning'> Detail</button></td>
                                            </tr>
                                            <tr id='accordion" . $aa['t_idtransaksi'] . "' class='collapse'>
                                                ...
                                                <td style='background-color: #C6E746'></td>
                                                <td style='text-align:right; background-color: #C6E746'>" . number_format($aa['t_jmlhdendapembayaran'], 0, ',', '.') . "</td>
                                                <td style='background-color: #C6E746'></td>
                                                <td style='text-align:center; background-color: #C6E746'>" . $t_tglbayardenda . "</td>
                                                <td style='text-align:right; background-color: #C6E746'>" . number_format($aa['t_jmlhbayardenda'], 0, ',', '.') . "</td>
                                                <td style='background-color: #C6E746'></td>
                                            </tr>
                                            ";
                }
    ...
    public function CariTransaksiByobjekAction()
    {
        ...
        if ($op['t_jenisobjek'] == 4 || $op['t_jenisobjek'] == 8) {
            ...
            $datatransaksipajak = $this->Tools()->getService('ObjekTable')->getTransaksibyIdObjek($data_get['idobjek'], $data_get['periodeobjek']);
