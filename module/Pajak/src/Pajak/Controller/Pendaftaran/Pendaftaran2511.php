...
//                        $newFile = $httpadapter->getFileName();
    ...
    public function FormEditAction() {
        ...
        if ($req->isGet()) {
            ...
            $form = new PendaftaranFrm($this->Tools()->getService('PendaftaranTable')->getcomboIdKecamatan(), $this->comboKelurahanCamat($data->t_kecamatan), null, $this->comboKelurahanCamat($data->t_kecamatan_badan));
            $form->bind($data);
        }
        $view = new ViewModel(array(
            'form' => $form,
...
//                        $newFile = $httpadapter->getFileName();
    ...
    public function editobjekpajakAction() {
        ...
        // var_dump($data);exit();
        $datakec = $this->Tools()->getService('PendaftaranTable')->getByKecamatancombo($data['t_kecamatanobjek']);
...
//                $res->setContent($opsi);
    ...
    public function cetakpendaftaranexcelAction() {
        ...
        $chars = array(0 => 'B', 1 => 'C', 2 => 'D', 3 => 'E', 4 => 'F', 5 => 'G', 6 => 'H', 7 => 'I', 8 => 'J', 9 => 'K', 10 => 'L',
...
//        ob_end_clean();
    ...
    public function cetakpendaftaranopexcelAction() {
        ...
        foreach ($records as $key => $arr_row) {
            switch ($arr_row) {
                case "t_npwpdwp": $datarow = "NPWPD";
                    break;
                case "t_namawp": $datarow = "Nama WP";
                    break;
                case "t_alamat_lengkapwp": $datarow = "Alamat WP";
                    break;
                case "t_tgldaftarobjek": $datarow = "Tgl. Daftar OP";
                    break;
                case "t_nop": $datarow = "NIOP";
                    break;
                case "t_namaobjek": $datarow = "Nama OP";
                    break;
                case "t_alamatlengkapobjek": $datarow = "Alamat OP";
                    break;
                case "t_kodeposobjek": $datarow = "Kode Pos OP";
                    break;
                case "t_notelpobjek": $datarow = "No. Telp. OP";
                    break;
                case "t_latitudeobjek": $datarow = "Latitude OP";
                    break;
                case "t_longitudeobjek": $datarow = "Longitude OP";
                    break;
                default:
                    $datarow = "";
            }
...
//        ob_end_clean();
    ...
    public function carirekeningAction() {
        ...
        $opsi = "";
        $opsi .= "<option value=''>Silahkan Pilih</option>";
        if ($data_get['t_jenisobjek'] == 1 || $data_get['t_jenisobjek'] == 2 || $data_get['t_jenisobjek'] == 3 || $data_get['t_jenisobjek'] == 5 || $data_get['t_jenisobjek'] == 7 || $data_get['t_jenisobjek'] == 8 || $data_get['t_jenisobjek'] == 9) {
            foreach ($datakorek as $r) {
                $opsi .= "<option value='" . $r['s_idkorek'] . "'>" . $r['korek'] . " || " . $r['s_namakorek'] . "</option>";
            }
        }
