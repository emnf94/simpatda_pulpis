...
class Penggabunganop extends AbstractActionController {
    ...
    public function dataGridAction() {
        ...
        $page = $base->page;
        $limit = $base->rows;
        $count = $this->Tools()->getService('PendaftaranTable')->getGridPenggabunganCount($base, $post);
        if ($count > 0 && $limit > 0) {
        ...
        if ($start < 0)
            $start = 0;
        $data = $this->Tools()->getService('PendaftaranTable')->getGridPenggabunganData($base, $start, $post);
        $s = "";
        ...
        foreach ($data as $row) {
            ...
            $s .= "<td data-title='#'><center>  <a href='penggabunganop/form_gabungobjek?t_idwp=$row[t_idwp]' class='btn btn-warning btn-xs' title='Gabungkan Objek Pajak'><i class='glyph-icon icon-unlink'></i></a></center></td>";
            $s .= "</tr>";
            $counter++;
        }
    ...
    public function FormGabungobjekAction() {
        $session = $this->getServiceLocator()->get('PajakService')->getStorage()->read();
        $req = $this->getRequest();
        if ($req->isGet()) {
            $id = $this->params("page");
            ...
            $form = new \Pajak\Form\Pendaftaran\PenggabunganopFrm();
            $form->bind($data);
        }
