...
namespace Pajak\Controller\Skpdkb;

use Zend\Mvc\Controller\AbstractActionController;
use Zend\View\Model\ViewModel;
use Pajak\Form\Skpdkb\SkpdkbFrm;
use Pajak\Model\Skpdkb\SkpdkbBase;
use Pajak\Form\Skpdkb\SkpdkbPembayaranFrm;
use Pajak\Model\Skpdkb\SkpdkbPembayaranBase;

class Skpdkb extends AbstractActionController {
    ...
    public function dataGridSkpdkbAction() {
        ...
        $page = $base->page;
        $limit = $base->rows;
        $count = $this->Tools()->getService('SkpdkbTable')->getGridCountSkpdkb($base);
        if ($count > 0 && $limit > 0) {
        ...
        if ($start < 0)
            $start = 0;
        $data = $this->Tools()->getService('SkpdkbTable')->getGridDataSkpdkb($base, $start);
        $s = "";
        ...
        foreach ($data as $row) {
            ...
            $s .= "<td data-title='No. SPTPD/SSPD' style='color:red'><center><b>" . $row['t_nourut'] . "/" . $row['t_nopembayaran'] . "</b></center></td>";
            ...
            $s .= "<td data-title='Tanggal SKPDKB'><center>" . date('d-m-Y', strtotime($row['t_tglskpdkb'])) . "</center></td>";
            $s .= "<td data-title='Jumlah Skpdkb' style='text-align: right'>" . number_format($row['t_totalskpdkb'], 0, ',', '.') . "</td>";
            $s .= "<td data-title='Jenis Pajak'>" . $row['s_namajenis'] . "</td>";
            $status_pemeriksaan = ($row['t_jenispemeriksaan'] == 1) ? '<span class="btn btn-xs btn-warning"><i class="glyph-icon icon-bank"></i> Pemeriksaan Kantor</span>' : '<span class="btn btn-xs btn-warning"><i class="glyph-icon icon-map-marker"></i> Pemeriksaan Lapangan</span>';
            $s .= "<td data-title='Jenis Pemeriksaan'>" . $status_pemeriksaan . "</td>";
            $hapus = "";
...
//            $pembayaran = "<a href='skpdkb/form_pembayaranskpdkb?t_idtransaksi=$row[t_idtransaksi]' class='btn btn-warning btn-xs btn-flat'><i class='glyph-icon icon-dollar'></i> Pembayaran</a>";
            $s .= "<td data-title='#'><center> <button onclick='bukaCetakSKPDKB($row[t_idtransaksi])' target='_blank' class='btn btn-primary btn-xs' title='Cetak SKPDKB'><i class='glyph-icon icon-print'></i></button> $hapus</center></td>";
            $s .= "</tr>";
            $counter++;
        }
    ...
    public function dataGridPembayaranSkpdkbAction() {
        $session = $this->getServiceLocator()->get('PajakService')->getStorage()->read();
        $allParams = (array) $this->getEvent()->getRouteMatch()->getParams();
        $base = new SkpdkbBase();
        $base->exchangeArray($allParams);
        ...
        $page = $base->page;
        $limit = $base->rows;
        $count = $this->Tools()->getService('SkpdkbTable')->getGridCountPembayaranSkpdkb($base);
        if ($count > 0 && $limit > 0) {
            $total_pages = ceil($count / $limit);
        } else {
            $total_pages = 0;
        ...
        if ($start < 0)
            $start = 0;
        $data = $this->Tools()->getService('SkpdkbTable')->getGridDataPembayaranSkpdkb($base, $start);
        $s = "";
        $counter = 1 + ($base->page * $base->rows) - $base->rows;
        if ($counter <= 1) {
            $counter = 1;
        ...
        foreach ($data as $row) {
            ...
            $s .= "<td data-title='NIOP'><center>" . $row['t_nop'] . "</center></td>";
            $s .= "<td data-title='Nama Objek'>" . $row['t_namaobjek'] . "</td>";
            $s .= "<td data-title='Tgl. Pembayaran'><center>" . date('d-m-Y', strtotime($row['t_tglbayarskpdkb'])) . "</center></td>";
            $s .= "<td data-title='Jml. Pembayaran' style='text-align: right'>" . number_format($row['t_jmlhbayarskpdkb'], 0, ',', '.') . "</td>";
            $s .= "</tr>";
            $counter++;
        }
        $datapaginge = new \Pajak\Controller\Plugin\Paginator();
    ...
    public function FormPembayaranskpdkbAction() {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($form->isValid()) {
                ...
                return $this->redirect()->toRoute('skpdkb');
            }
        }
        $view = new ViewModel(array(
            'form' => $form,
    ...
    public function hapusAction() {
         ...
         */
        $session = $this->getServiceLocator()->get('PajakService')->getStorage()->read();
        $this->Tools()->getService('SkpdkbTable')->hapusSkpdkb($this->params('page'), $session);
        return $this->getResponse();
    }

    public function cetakDataSkpdkbAction() {
        $req = $this->getRequest();
        $data_get = $req->getQuery();
        $data = $this->Tools()->getService('SkpdkbTable')->getDataSkpdkb($data_get->tglbayar0, $data_get->tglbayar1, $data_get->t_kecamatan, $data_get->t_kelurahan, $data_get->t_idkorek);
        $ar_pemda = $this->Tools()->getService('PemdaTable')->getdata();
    ...
    public function dataSkpdkbAction() {
        ...
        $data = $this->Tools()->getService('SkpdkbTable')->getTransaksiSKPDKBByIdTransaksi($data_get['idtransaksi']);
        $data_render = array(
            "idtransaksi" => $data['t_idtransaksi'],
            "namawp" => $data['t_nama'],
            "alamatwp" => $data['t_alamat'],
            "tglskpdkb" => date('d-m-Y', strtotime($data['t_tglskpdkb'])),
        );
        return $this->getResponse()->setContent(\Zend\Json\Json::encode($data_render));
    }
